...
double Extraction_a(const std::string& equation) {
    int a;
...
double Extraction_c(const std::string& equation) {
    int c;
    int c_str;
    std::string eq = equation; // сохраняем исходную строку
    removeSpaces(eq); // удаляем пробелы из строки
...
int main() {
    ...
    int a = Extraction_a(equation);
    int b = Extraction_b(equation);
    int c = Extraction_c(equation);
